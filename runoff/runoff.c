...
int find_min(void)
{
    // Initialize min_votes to the vote count of the first candidate
    int min_vote = candidates[0].votes;

    // Iterate through each candidate
    for (int i = 0; i < candidate_count; i++)
    {
        // Check for candidate who is still running
        if (!candidates[i].eliminated)
        {
...
bool is_tie(int min)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // Check if candidate is not eliminated and has diff no of votes than min
    ...
    // If all remaining candidates have the same number of votes as min, its a tie
    return true;
}
...
void eliminate(int min)
{
    for (int i = 0; i < candidate_count; i++)
    {
        // Check for candidate with minimum votes
